    ...
    <el-row type="flex">
      <el-col :span="20">
        <div class="divClass70320102">
          ...
          <div v-if="tabType == 0">
            <el-row>
              <el-col :span="17" :offset="1" class="colClass70320201">
                <el-form
                  ref="ruleFormDx"
                  :model="ruleFormDx"
                  label-width="100px"
                  ...
                  <el-form-item>
                    <el-radio-group v-model="radioOption" size="medium" align="left" @change="DxSelectChange">
                      <el-row>
                        <el-radio :label="OptionA" class="el_radio70320101" />
                      </el-row>
                      <el-row align="left">
                        <el-radio :label="OptionB" class="el_radio70320101"/>
                      </el-row>
                      <el-row align="left">
                        <el-radio :label="OptionC" class="el_radio70320101"/>
                      </el-row>
                      <el-row align="left">
                        <el-radio :label="OptionD" class="el_radio70320101"/>
                      </el-row>
                      <el-row align="left">
                        <el-radio :label="OptionE" class="el_radio70320101"/>
                      </el-row>
                    </el-radio-group>
                  </el-form-item>
                </el-form>
...
export default {
  ...
  mounted: function() {
    ...
    if (params) {
      this.listMap1 = this.$route.params.listMap1
      this.totalScore = this.$route.params.totalScore
      this.TestTime = this.$route.params.TestTime
      const lstMap = generateLstMap(this.listMap1, this)
      ...
      this.$http(url, params).then( // 通讯后台修改数据库中信息
        res => {
          ...
          console.log('703201getQuestionByRandomAlgm:' + JSON.stringify(_data))
          if (_data.data.TrxCode === '000000') {
            const pageInfo = _data.data.PageInfo
            this.questionByRandomAlgm = pageInfo
            for (let j = 0; j < pageInfo.length; j++) {
              // 考题表格信息加载
              this.AnswertableData.push({
                an_ID: pageInfo[j].ID,
                an_TYPE: pageInfo[j].TYPE,
                an_BUSITYPE: pageInfo[j].an_BUSITYPE,
                an_EXAMANSWER: '',
                an_ANSWER: pageInfo[j].ANSWER,
                an_SCORE: this.scoreMap.get(pageInfo[j].TYPE)
              })
              // 考题分装
