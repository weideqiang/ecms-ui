    ...
    <el-main>
      ...
      <el-row>
        <el-form
          ...
          size="small">
          <el-col :span="11">
            <el-form-item label="策略名称：" prop="Cl_ClMc">
              <el-input v-model="ruleFormCL.Cl_ClMc" clearable/>
            </el-form-item>
          </el-col>
          <el-col :span="3">
            <el-form-item>
              <el-button size="small" type="primary" @click="submitForm()">生成策略</el-button>
            </el-form-item>
          </el-col>
          <el-col :span="3">
            <el-form-item>
...
<script>
export default {
  name: 'T7005', // 依赖npm install xe-utils vxe-table
  data: function() {
  ...
  methods: {
    submitForm() {
      ...
      if (this.ruleFormDx.dx_hjFz === '0' && this.ruleFormDux.dux_hjFz === '0' && this.ruleFormPd.Pd_hjFz === '0') {
        this.$message.error('请配置策略！')
        return
      }
      const map = { 'zoneno': this.CurrentZoneNo, 'brno': this.CurrentBranchNo,
        'tlrno': this.CurrentUserNo, 'accdate': this.CurrentSystemDate,
        'insertDataDx': DataRecords_DX, 'insertDataDux': DataRecords_DuX,
        'insertDataPd': DataRecords_PD,
        'title': this.ruleFormCL.Cl_ClMc, 'score': this.ruleFormHZ.Hz_Zf,
        'DxFZ': this.ruleFormDx.dx_hjFz, 'DuoXFz': this.ruleFormDux.dux_hjFz,
        'PdFz': this.ruleFormPd.Pd_hjFz }
      const params = { 'ruleForm': JSON.stringify(map) }
      const url = this.$examSystemMngApi.insertEFS_EXAM_SELECTMANAGER
