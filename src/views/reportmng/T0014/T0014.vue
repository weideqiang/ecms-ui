  ...
  <el-main>
    ...
    <el-row>
      <el-table
        ...
        <el-table-column prop="tlrno" label="柜员代码">
          <template slot-scope="scope">
            <template v-if="scope.row.editing">
              <el-input v-model="scope.row.tlrno" class="edit-input" placeholder="机构代码"/>
            </template>
        ...
        <el-table-column prop="avgtime1" label="柜员交易平均时长(秒)">
          <template slot-scope="scope">
            <template v-if="scope.row.editing">
        ...
        <el-table-column prop="avgtime2" label="全行交易平均时长(秒）">
          <template slot-scope="scope">
            <template v-if="scope.row.editing">
  ...
  methods: {
    ...
    login: function() { // 查询
      this.$refs.form.validate(valid => {
        if (valid) {
          const params = { 'pageNum': this.currentPage, 'pageSize': this.pagesize, 'sltMonth': this.inputForm.sltMonth, 'brno': this.inputForm.brno, 'trxno': this.inputForm.trxno }
          this.$http(this.$reportMngApi.getTrlTrxAvgMongthTime, params).then(
            res => {
    ...
    derive: function() {
      ...
      const params = { 'brno': this.inputForm.brno, 'accdate': '', 'trxno': this.inputForm.trxno, 'sltMonth': this.inputForm.sltMonth, 'dealNumber': 'T0014' }
    ...
    download(data) {
      ...
      link.href = url
      link.setAttribute('download', this.inputForm.sltMonth + '柜员交易平均时间日报表' + '.xlsx')
