  ...
  <el-main>
    <div class="demo-form-div">
      ...
      <div class="dialog-footer" style="width: 100%;" align="right">
        <el-row>
          <el-col :span="8">
            <el-button size="medium" type="primary" @click="exitTrade()">{{ '返回' }}</el-button>
          </el-col>
          <el-col :span="8">
    ...
    dataTable() {
      ...
      if (this.device === 'mobile') { // 移动端
        ...
        const tr = [
          ...
          { id: '3', label: '客户账号', prop: 'CUSTACC' },
          { id: '4', label: '客户名称', prop: 'CUSTNAME' },
          { id: '5', label: '登记信息', prop: 'DOCTYPE' },
          { id: '6', label: '备注', prop: 'CAUSE' },
          { id: '7', label: '柜员编号（登记时）', prop: 'TELLERNO2' },
          { id: '8', label: '登记流水号', prop: 'MAINSERIALNO2' },
          { id: '9', label: '领取日期', prop: 'RECEIVEDATE' },
          { id: '10', label: '领取时间', prop: 'RECEIVETIME' },
          { id: '11', label: '柜员号（领取时）', prop: 'TELLERNO1' },
          { id: '12', label: '领取流水号', prop: 'MAINSERIALNO1' },
          { id: '13', label: '登记类型', prop: 'OPERATION' },
          { id: '14', label: '网点编号', prop: 'BRNO' },
          { id: '15', label: '网点名称', prop: 'BRNAME' }
        ]
        const expands = [
          {
            label: '机构号：',
  ...
  methods: {
    ...
    export2Excel() { // 需要安装依赖 npm install -S file-saver xlsx以及npm install -D script-loader
      require.ensure([], () => {
        ...
        const tHeader = ['日期', '时间', '客户账号', '客户名称', '登记信息', '备注', '柜员编号(登记时)', '登记流水号',
          '领取日期', '领取时间', '柜员号(领取时)', '领取流水号', '登记类型', '网点编号', '网点名称']// 表头
        // 表头对应字段名，要和下面数据key对应
        const filterVal = ['TBACKDATE', 'TBACKTIME', 'CUSTACC', 'CUSTNAME', 'DOCTYPE', 'CAUSE', 'TELLERNO2', 'MAINSERIALNO2',
          'RECEIVEDATE', 'RECEIVETIME', 'TELLERNO1', 'MAINSERIALNO1', 'OPERATION', 'BRNO', 'BRNAME']
        const list = this.tableData // 数据来源
        const data = this.formatJson(filterVal, list) // 数据格式化
        var index1 = '客户单证领取、收回登记簿'// 导出时文件名
    ...
    queryTDataByPageSizeAndNum(pageNum, pageSize) {
      ...
      const map = { 'acctNo': this.$route.params.acctNo,
    ...
    onRowdbClick: function(row) { // 表格双击事件
      ...
      this.$router.push({
        ...
        params: {
          __map: row,
          __tzType: '0',
          __OPERATION: row.row.OPERATION === '0-查询' ? '0' : (row.row.OPERATION === '1-领取' ? '1'
            : (row.row.OPERATION === '2-登记' ? '2' : row.row.OPERATION))
        }
      })
    },
