    ...
    dataTable() {
      ...
      if (this.device === 'mobile') { // 移动端
        ...
        const tr = [
          ...
          { id: '2', label: '交接时间', prop: 'HANDOVERTIME' },
          { id: '3', label: '场次', prop: 'INTHEDAY' },
          { id: '4', label: '操作方式', prop: 'OPERATEWAY' },
          { id: '5', label: '包（票据）、数量', prop: 'BILLNUMBER' },
          { id: '6', label: '交换行号', prop: 'EXCHANGENUMBER' },
          { id: '7', label: '卡（锁）片号', prop: 'CARDNUMBER' },
          { id: '8', label: '借方金额', prop: 'DEBITAMOUNT' },
          { id: '9', label: '贷方金额', prop: 'CREDITAMOUNT' },
          { id: '10', label: '包（票据）状况', prop: 'BILLSTATUS' },
          { id: '11', label: '交出人', prop: 'HANDOVER' },
          { id: '12', label: '接收人', prop: 'SENDEE' },
          { id: '13', label: '机构号', prop: 'BRNO' },
          { id: '14', label: '机构名称', prop: 'BRNAME' },
          { id: '15', label: '柜员号', prop: 'TELLERNO' },
          { id: '16', label: '柜员名称', prop: 'TELLERNAME' },
          { id: '17', label: '流水号', prop: 'MAINSERIALNO' },
          { id: '18', label: '说明', prop: 'ILLUSTRATE' },
          { id: '19', label: '维护状态', prop: 'WFLAG' }
        ]
        const expands = [
          {
            label: '机构号：',
  ...
  methods: {
    ...
    export2Excel() { // 需要安装依赖 npm install -S file-saver xlsx以及npm install -D script-loader
      require.ensure([], () => {
        ...
        const tHeader = ['交接日期', '交接时间', '场次', '操作方式', '包（票据）、数量', '交换行号', '卡（锁）片号', '借方金额', '贷方金额', '包（票据）状况', '交出人', '接收人', '机构号', '机构名称', '柜员号', '柜员名称', '流水号', '说明', '维护状态']// 表头
        // 表头对应字段名，要和下面数据key对应
        const filterVal = ['HANDOVERDATE', 'HANDOVERTIME', 'INTHEDAY', 'OPERATEWAY', 'BILLNUMBER', 'EXCHANGENUMBER', 'CARDNUMBER', 'DEBITAMOUNT', 'CREDITAMOUNT', 'BILLSTATUS', 'HANDOVER', 'SENDEE', 'BRNO', 'BRNAME', 'TELLERNO', 'TELLERNAME', 'MAINSERIALNO', 'ILLUSTRATE', 'WFLAG']
        const list = this.tableData // 数据来源
        const data = this.formatJson(filterVal, list) // 数据格式化
        var index1 = '票据交接登记簿'// 导出时文件名
    ...
    queryTDataByPageSizeAndNum(pageNum, pageSize) {
      console.log('T5508 getXRPISBILLHANDOVERREGISTER :' + pageNum + '|' + pageSize)
      const params1 = { STARTDATE: this.$route.params.STARTDATE,
        ...
        INTHEDAY: this.$route.params.INTHEDAY,
        OPERATEWAY: this.$route.params.OPERATEWAY,
        BILLNUMBER: this.$route.params.BILLNUMBER,
        EXCHANGENUMBER: this.$route.params.EXCHANGENUMBER,
        CARDNUMBER: this.$route.params.CARDNUMBER,
        DEBITAMOUNT: this.$route.params.DEBITAMOUNT,
        CREDITAMOUNT: this.$route.params.CREDITAMOUNT,
        BILLSTATUS: this.$route.params.BILLSTATUS,
        HANDOVER: this.$route.params.HANDOVER,
        SENDEE: this.$route.params.SENDEE,
        'currentPage': pageNum, 'pageSize': pageSize }
      const url1 = this.$reportMngApi.getXRPISBILLHANDOVERREGISTER
