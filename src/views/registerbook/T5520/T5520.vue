  ...
  <el-main>
    <div>
      <el-form
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item ref="OpenChanel" label="开户渠道：" prop="accountOpenChanel">
...
export default {
  name: 'T5520',
  data() {
    const validOpenChanel = (rule, value, callback) => {
      // if (value !== '') {
      //   this.$router.push({
      //     path: this.redirect || '/T552001',
      //     name: 'T552001',
      //     params: {
      //       khFlag: '0'
      //     }
      //   })
      // } else {
      //   callback(new Error('开户渠道不可为空,请重新选择！'))
      //   this.$refs.OpenChanel.focus()
      // }
    }
    return {
      ruleForm: {
      ...
      rules: {
        accountOpenChanel: [
          ...
          { trigger: 'change', validator: validOpenChanel }
        ],
        startDate: [
  ...
  computed: {
    ...
    dataTable() {
      ...
      if (this.device === 'mobile') { // 移动端
        ...
        const tr = [
          { id: '1', label: '客户预约日期', prop: 'APPLEDATE' },
          { id: '2', label: '审核通过日期', prop: 'AUDITINGDATE' },
          { id: '3', label: '存款人名称', prop: 'CUSTOMERNAME' },
          { id: '4', label: '登记证书号码', prop: 'THEDOCUMENTSERIALNO' },
          { id: '5', label: '电话号码', prop: 'PHONENUMBER' },
  ...
  methods: {
    ...
    queryTDataByPageSizeAndNum(pageNum, pageSize) {
      console.log('T5520 queryTDataByPageSizeAndNum :' + pageNum + '|' + pageSize)
      const map = {
        'openingchannel': this.ruleForm.accountOpenChanel, 'gwauditstatus': '2',
