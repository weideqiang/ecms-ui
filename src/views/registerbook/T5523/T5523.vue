...
export default {
    ...
    dataTable() {
      ...
      if (this.device === 'mobile') { // 移动端
        ...
        const tr = [
          ...
          { id: '5', label: '开户许证可日期', prop: 'NOTE32' }
        ]
        const expands = [
          {
            label: '机构号：',
  ...
  methods: {
    queryTDataByPageSizeAndNum(pageNum, pageSize) {
      console.log('T5523 queryTDataByPageSizeAndNum :' + pageNum + '|' + pageSize)
      const map = {
        'openingchannel': this.ruleForm.accountOpenChanel, 'gwauditstatus': '8',
