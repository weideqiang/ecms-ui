...
export default {
  ...
  data() {
    return {
      ...
      INVENTORYTYPES: this.$combo.PZLXTypes5550
    }
  },
  computed: {
    dataTable() {
      ...
      if (this.device === 'mobile') { // 移动端
        ...
        const tr = [
          ...
          { id: '12', label: '交易代码', prop: 'TRADECODE' },
          { id: '13', label: '交易名称', prop: 'TRADENAME' },
          { id: '14', label: '交易金额', prop: 'AMOUNT' },
          { id: '15', label: '收款人账号', prop: 'PAYEEACC' },
          { id: '16', label: '收款人户名', prop: 'PAYEENAME' },
          { id: '17', label: '结算方式', prop: 'PAYTYPE' },
          { id: '18', label: '凭证类型', prop: 'INVENTORYTYPE' },
          { id: '19', label: '凭证号码', prop: 'INVENTORYCODE' },
          { id: '20', label: '查证联系人1', prop: 'FCHECKPERSON' },
          { id: '21', label: '查证联系人电话1', prop: 'FCHECKPHONE' },
          { id: '22', label: '查证联系人2', prop: 'SCHECKPERSON' },
          { id: '23', label: '查证联系人电话2', prop: 'SCHECKPHONE' },
          { id: '24', label: '查证结果', prop: 'CHECKRESULT' },
          { id: '25', label: '审批员', prop: 'CHECKUSERNO' },
          { id: '26', label: '说明', prop: 'NOTE1' }
        ]
        const expands = [
          {
            label: '机构号：',
  ...
  mounted: function() {
    ...
    if (params) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        this.$set(list[i], 'INVENTORYTYPE', this.selectLable(list[i].INVENTORYTYPE, this.INVENTORYTYPES))
      }
      this.tableData = list
    ...
    export2Excel() { // 需要安装依赖 npm install -S file-saver xlsx以及npm install -D script-loader
      require.ensure([], () => {
        ...
        const tHeader = ['登记日期', '登记时间', '地区号', '网点号', '柜员号', 'FCR流水号', '主流水号', '前端处理流水号',
          '币种', '账号', '户名', '交易代码', '交易名称', '交易金额', '收款人账号', '收款人户名', '结算方式', '凭证类型',
          '凭证号码', '查证联系人1', '查证联系人电话1', '查证联系人2', '查证联系人电话2', '查证结果', '审批员', '说明']// 表头
        // 表头对应字段名，要和下面数据key对应
        const filterVal = ['TRADEDATE', 'TRADETIME', 'ZONENO', 'BRANCHNO', 'USERNO', 'FCRSERIALNO', 'MAINSERIALNO',
          'SERIALNO', 'CURRENCYTYPE', 'ACCNO', 'ACCNAME', 'TRADECODE', 'TRADENAME', 'AMOUNT', 'PAYEEACC', 'PAYEENAME',
          'PAYTYPE', 'INVENTORYTYPE', 'INVENTORYCODE', 'FCHECKPERSON', 'FCHECKPHONE', 'SCHECKPERSON', 'SCHECKPHONE',
          'CHECKRESULT', 'CHECKUSERNO', 'NOTE1']
        const list = this.tableData // 数据来源
        const data = this.formatJson(filterVal, list) // 数据格式化
        var index1 = '大额付款联系查证登记簿'// 导出时文件名
