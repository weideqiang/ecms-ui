  ...
  computed: {
    dataTable() {
      ...
      if (this.device === 'mobile') { // 移动端
        ...
        const tr = [
          { id: '1', label: '检查日期', prop: 'CHECKDATE' },
          { id: '2', label: '检查起始时间', prop: 'CHECKSTARTTIME' },
          { id: '3', label: '检查结束时间', prop: 'CHECKENDTIME' },
          { id: '4', label: '现金管理', prop: 'CASHMANAGEMENT' },
          { id: '5', label: '印章管理', prop: 'SEALMANEGEMENT' },
          { id: '6', label: '重要机具管理', prop: 'IMPLEMENTMANAGEMENT' },
          { id: '7', label: '重要物品', prop: 'IMPORTANTITEMS' },
          { id: '8', label: '被检查人1', prop: 'INSPECTED' },
          { id: '9', label: '检查人1', prop: 'RUMMAGER' },
          { id: '10', label: '检查人2', prop: 'PERSONINCHARGE' },
          { id: '11', label: '处理意见', prop: 'HANDLINGSUGGESTION' },
          { id: '12', label: '机构号', prop: 'BRNO' },
          { id: '13', label: '机构名称', prop: 'BRNAME' },
          { id: '14', label: '柜员号', prop: 'TELLERNO' },
          { id: '15', label: '柜员名称', prop: 'TELLERNAME' },
          { id: '16', label: '流水号', prop: 'MAINSERIALNO' },
          { id: '17', label: '说明', prop: 'ILLUSTRATE' },
          { id: '18', label: '维护标志', prop: 'WFLAG' }
        ]
        const expands = [
          {
            label: '机构号：',
  ...
  methods: {
    ...
    export2Excel() { // 需要安装依赖 npm install -S file-saver xlsx以及npm install -D script-loader
      require.ensure([], () => {
        ...
        const tHeader = ['检查日期', '检查起始时间', '检查结束时间', '现金管理', '印章管理', '重要机具管理', '重要物品', '被检查人1', '检查人1', '检查人2', '处理意见', '机构号', '机构名称', '柜员号', '柜员名称', '流水号', '说明', '维护标志']// 表头
        // 表头对应字段名，要和下面数据key对应
        const filterVal = ['CHECKDATE', 'CHECKSTARTTIME', 'CHECKENDTIME', 'CASHMANAGEMENT', 'SEALMANEGEMENT', 'IMPLEMENTMANAGEMENT', 'IMPORTANTITEMS', 'INSPECTED', 'RUMMAGER', 'PERSONINCHARGE', 'HANDLINGSUGGESTION', 'BRNO', 'BRNAME', 'TELLERNO', 'TELLERNAME', 'MAINSERIALNO', 'ILLUSTRATE', 'WFLAG']
        const list = this.tableData // 数据来源
        const data = this.formatJson(filterVal, list) // 数据格式化
        var index1 = '检查登记簿'// 导出时文件名
    ...
    queryTDataByPageSizeAndNum(pageNum, pageSize) {
      console.log('T5509 getXRPISCHECKREGISTER :' + pageNum + '|' + pageSize)
      const params1 = { STARTDATE: this.$route.params.STARTDATE,
        ...
        CASHMANAGEMENT: this.$route.params.CASHMANAGEMENT,
        SEALMANEGEMENT: this.$route.params.SEALMANEGEMENT,
        IMPLEMENTMANAGEMENT: this.$route.params.IMPLEMENTMANAGEMENT,
        INSPECTED: this.$route.params.INSPECTED,
        RUMMAGER: this.$route.params.RUMMAGER,
        PERSONINCHARGE: this.$route.params.PERSONINCHARGE,
        'currentPage': pageNum, 'pageSize': pageSize }
      const url1 = this.$reportMngApi.getXRPISCHECKREGISTER
