  ...
  computed: {
    dataTable() {
      ...
      if (this.device === 'mobile') { // 移动端
        ...
        const tr = [
          { id: '1', label: '出售日期', prop: 'SALEDATE' },
          { id: '2', label: '出售时间', prop: 'SALETIME' },
          { id: '3', label: '凭证信息', prop: 'CERTTYPE' },
          { id: '4', label: '领用单位户名', prop: 'UNITNAME' },
          { id: '5', label: '领用单位账号', prop: 'UNITACC' },
          { id: '6', label: '领用人', prop: 'COLLARMAN' },
          { id: '7', label: '机构号', prop: 'BRNO' },
          { id: '8', label: '机构名称', prop: 'BRNAME' },
          { id: '9', label: '柜员号', prop: 'TELLERNO' },
          { id: '10', label: '柜员名称', prop: 'TELLERNAME' },
          { id: '11', label: '流水号', prop: 'MAINSERIALNO' },
          { id: '12', label: '说明', prop: 'ILLUSTRATE' }
        ]
        const expands = [
          {
            label: '机构号：',
  ...
  methods: {
    ...
    export2Excel() { // 需要安装依赖 npm install -S file-saver xlsx以及npm install -D script-loader
      require.ensure([], () => {
        ...
        const tHeader = ['出售日期', '出售时间', '凭证信息', '领用单位户名', '领用单位账号', '领用人', '机构号', '机构名称', '柜员号', '柜员名称', '流水号', '说明']// 表头
        // 表头对应字段名，要和下面数据key对应
        const filterVal = ['SALEDATE', 'SALETIME', 'CERTTYPE', 'UNITNAME', 'UNITACC', 'COLLARMAN', 'BRNO', 'BRNAME', 'TELLERNO', 'TELLERNAME', 'MAINSERIALNO', 'ILLUSTRATE']
        const list = this.tableData // 数据来源
        const data = this.formatJson(filterVal, list) // 数据格式化
        var index1 = '重要空白凭证出售登记簿'// 导出时文件名
    ...
    queryTDataByPageSizeAndNum(pageNum, pageSize) {
      console.log('T5503 getXRPISCERTSALEREGISTER :' + pageNum + '|' + pageSize)
      const params1 = { 'STARTDATE': this.$route.params.STARTDATE, 'ENDDATE': this.$route.params.ENDDATE, 'BRNO': this.$route.params.BRNO, 'BRANCHLEVEL': this.$route.params.BRANCHLEVEL, 'TELLERNO': this.$route.params.TELLERNO,
        'CERTTYPE': this.$route.params.CERTTYPE, 'CERTSTARTNUMBER': this.$route.params.CERTSTARTNUMBER, 'CERTENDNUMBER': this.$route.params.CERTENDNUMBER, 'SHOWINNUMBER': this.$route.params.SHOWINNUMBER,
        'SALENUMBER': this.$route.params.SALENUMBER, 'UNITNAME': this.$route.params.UNITNAME, 'UNITACC': this.$route.params.UNITACC, 'COLLARMAN': this.$route.params.COLLARMAN, 'currentPage': pageNum, 'pageSize': pageSize }
      const url1 = this.$reportMngApi.getXRPISCERTSALEREGISTER
