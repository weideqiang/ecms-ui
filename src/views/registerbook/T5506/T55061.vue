    ...
    dataTable() {
      ...
      if (this.device === 'mobile') { // 移动端
        ...
        const tr = [
          ...
          { id: '2', label: '来源', prop: 'LAIYUAN' },
          { id: '3', label: 'ATM机编号', prop: 'ATMNUMBER' },
          { id: '4', label: '客户名称', prop: 'CUSTNAME' },
          { id: '5', label: '账号或卡号', prop: 'ACCOUNTNUMBER' },
          { id: '6', label: '金额', prop: 'MONEY' },
          { id: '7', label: '差错原因', prop: 'ERRORCAUSE' },
          { id: '8', label: '处理情况', prop: 'DEALSITUATION' },
          { id: '9', label: '机构号', prop: 'BRNO' },
          { id: '10', label: '机构名称', prop: 'BRNAME' },
          { id: '11', label: '柜员号', prop: 'TELLERNO' },
          { id: '12', label: '柜员名称', prop: 'TELLERNAME' },
          { id: '13', label: '流水号', prop: 'MAINSERIALNO' },
          { id: '14', label: '说明', prop: 'ILLUSTRATE' }
        ]
        const expands = [
          {
            label: '机构号：',
  ...
  mounted: function() {
    ...
    if (params) {
      ...
      for (let i = 0; i < list.length; i++) {
        this.$set(list[i], 'LAIYUAN', this.selectLable(list[i].LAIYUAN, this.LAIYUANS))
      }
      this.tableData = list
    }
  },
  methods: {
    ...
    export2Excel() { // 需要安装依赖 npm install -S file-saver xlsx以及npm install -D script-loader
      require.ensure([], () => {
        ...
        const tHeader = ['登记日期', '来源', 'ATM机编号', '客户名称', '账号或卡号', '金额', '差错原因', '处理情况', '机构号', '机构名称', '柜员号', '柜员名称', '流水号', '说明']// 表头
        // 表头对应字段名，要和下面数据key对应
        const filterVal = ['REGISTERDATE', 'LAIYUAN', 'ATMNUMBER', 'CUSTNAME', 'ACCOUNTNUMBER', 'MONEY', 'ERRORCAUSE', 'DEALSITUATION', 'BRNO', 'BRNAME', 'TELLERNO', 'TELLERNAME', 'MAINSERIALNO', 'ILLUSTRATE']
        const list = this.tableData // 数据来源
        const data = this.formatJson(filterVal, list) // 数据格式化
        var index1 = '客户存取款差错登记簿'// 导出时文件名
    ...
    queryTDataByPageSizeAndNum(pageNum, pageSize) {
      console.log('T5506 getXRPISCUSTACCMONEYREGISTER :' + pageNum + '|' + pageSize)
      const params1 = { 'STARTDATE': this.$route.params.STARTDATE,
        'ENDDATE': this.$route.params.ENDDATE,
        'LAIYUAN': this.$route.params.LAIYUAN,
        ...
        'ERRORCAUSE': this.$route.params.ERRORCAUSE,
        'DEALSITUATION': this.$route.params.DEALSITUATION,
        'ATMNUMBER': this.$route.params.ATMNUMBER,
        ...
        res1 => {
          ...
          if (_data1.code === 'SUCCESS') {
            if (_data1.data.TrxCode === '000000') {
              ...
              if (_data1.data.TrxCode === 'T5506_01') {
