...
export default {
    ...
    dataTable() {
      ...
      if (this.device === 'mobile') { // 移动端
        ...
        const tr = [
          ...
          { id: '4', label: '流水号', prop: 'MAINSERIALNO' }
        ]
        const expands = [
          {
            label: '机构号：',
  ...
  methods: {
    queryTDataByPageSizeAndNum(pageNum, pageSize) {
      console.log('T5522 queryTDataByPageSizeAndNum :' + pageNum + '|' + pageSize)
      const map = {
        'openingchannel': this.ruleForm.accountOpenChanel, 'gwauditstatus': '6',
