  ...
  computed: {
    dataTable() {
      ...
      if (this.device === 'mobile') { // 移动端
        ...
        const tr = [
          { id: '1', label: '日期', prop: 'WORKDATE' },
          { id: '2', label: '部门', prop: 'BRNO' },
          { id: '3', label: '交易流水', prop: 'MAINSERIALNO' },
          { id: '4', label: '开销户标志', prop: 'STATUS' },
          { id: '5', label: '卡账号', prop: 'ACCOUNT' },
          { id: '6', label: '户名', prop: 'CUSTNAME' },
          { id: '7', label: '证件类型', prop: 'IDTYPE' },
          { id: '8', label: '科目', prop: 'ACCTYPE' },
          { id: '9', label: '产品名称', prop: 'PROCODE' },
          { id: '10', label: '账户属性', prop: 'ACCATTRIBUTE' },
          { id: '11', label: '开户类别', prop: 'CUSTTYPE' },
          { id: '12', label: '账户性质', prop: 'ACCSTATE' },
          { id: '13', label: '存期', prop: 'TERM' },
          { id: '14', label: '开户日期', prop: 'ODATE' },
          { id: '15', label: '核准日期', prop: 'AUTHDATE' },
          { id: '16', label: '金额', prop: 'AMOUNT' },
          { id: '17', label: '代理人姓名', prop: 'AGTNAME' },
          { id: '18', label: '代理人证件类型', prop: 'AGTIDTYPE' },
          { id: '19', label: '代理人联系地址', prop: 'AGTADRESS' },
          { id: '20', label: '代理人联系电话', prop: 'AGTCUSTPHONE' },
          { id: '21', label: '联网核查结果', prop: 'HR' },
          { id: '22', label: '操作员', prop: 'TELLERNO' },
          { id: '23', label: '审批员', prop: 'AUTHTELLER' }
        ]
        const expands = [
          {
            label: '机构号：',
  ...
  mounted: function() {
    ...
    if (params) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        this.$set(list[i], 'AGTIDTYPE', this.selectLable(list[i].AGTIDTYPE, this.IDTYPES))
      }
      this.tableData = list
  ...
  methods: {
    ...
    export2Excel() { // 需要安装依赖 npm install -S file-saver xlsx以及npm install -D script-loader
      require.ensure([], () => {
        ...
        const tHeader = ['日期', '部门', '交易流水', '开销户标志', '卡账号', '户名', '证件类型', '科目', '产品名称', '账户属性',
          '开户类别', '账户性质', '存期', '开户日期', '核准日期', '金额', '代理人姓名', '代理人证件类型', '代理人联系地址',
          '代理人联系电话', '联网核查结果', '操作员', '审批员'
        ]// 表头
        // 表头对应字段名，要和下面数据key对应
        const filterVal = ['WORKDATE', 'BRNO', 'MAINSERIALNO', 'STATUS', 'ACCOUNT', 'CUSTNAME', 'IDTYPE', 'ACCTYPE',
          'PROCODE', 'ACCATTRIBUTE', 'CUSTTYPE', 'ACCSTATE', 'TERM', 'ODATE', 'AUTHDATE', 'AMOUNT', 'AGTNAME',
          'AGTIDTYPE', 'AGTADRESS', 'AGTCUSTPHONE', 'HR', 'TELLERNO', 'AUTHTELLER'
        ]
        const list = this.tableData // 数据来源
        const data = this.formatJson(filterVal, list) // 数据格式化
        var index1 = '港澳台居民开销户登记簿-客户查询'// 导出时文件名
