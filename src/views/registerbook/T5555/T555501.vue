  ...
  computed: {
    dataTable() {
      ...
      if (this.device === 'mobile') { // 移动端
        ...
        const tr = [
          { id: '1', label: '开户日期', prop: 'TRADEDATE' },
          { id: '2', label: '客户类别', prop: 'CUSTTYPE' },
          { id: '3', label: '客户号', prop: 'CUSTID' },
          { id: '4', label: '客户姓名', prop: 'CUSTNAME' },
          { id: '5', label: '证件种类', prop: 'IDTYPE' },
          { id: '6', label: '证件号码', prop: 'IDNO' },
          { id: '7', label: '通行证号', prop: 'RESIDENO' },
          { id: '8', label: '操作柜员', prop: 'USERNO' }
        ]
        const expands = [
          {
            label: '机构号：',
  ...
  methods: {
    ...
    export2Excel() { // 需要安装依赖 npm install -S file-saver xlsx以及npm install -D script-loader
      require.ensure([], () => {
        ...
        const tHeader = ['开户日期', '客户类别', '客户号', '客户姓名', '证件种类', '证件号码', '通行证号', '操作柜员'
        ]// 表头
        // 表头对应字段名，要和下面数据key对应
        const filterVal = ['TRADEDATE', 'CUSTTYPE', 'CUSTID', 'CUSTNAME', 'IDTYPE', 'IDNO', 'RESIDENO', 'USERNO'
        ]
