    ...
    dataTable() {
      ...
      if (this.device === 'mobile') { // 移动端
        ...
        const tr = [
          ...
          { id: '2', label: '登记时间', prop: 'RETISTERTIME' },
          { id: '3', label: '入库时间', prop: 'STOCKINTIME' },
          { id: '4', label: '出库时间', prop: 'STOCKOUTTIME' },
          { id: '5', label: '姓名', prop: 'NAMES' },
          { id: '6', label: '人数', prop: 'NUMBERS' },
          { id: '7', label: '事由', prop: 'PARTICULAR' },
          { id: '8', label: '库管员1', prop: 'STOREKEEPER1' },
          { id: '9', label: '库管员2', prop: 'STOREKEEPER2' },
          { id: '10', label: '批准人', prop: 'APPROVER' },
          { id: '11', label: '机构号', prop: 'BRNO' },
          { id: '12', label: '机构名称', prop: 'BRNAME' },
          { id: '13', label: '柜员号', prop: 'TELLERNO' },
          { id: '14', label: '柜员名称', prop: 'TELLERNAME' },
          { id: '15', label: '交易流水号', prop: 'MAINSERIALNO' },
          { id: '16', label: '维护状态', prop: 'WFLAG' }
        ]
        const expands = [
          {
            label: '机构号：',
  ...
  methods: {
    ...
    export2Excel() { // 需要安装依赖 npm install -S file-saver xlsx以及npm install -D script-loader
      require.ensure([], () => {
        ...
        const tHeader = ['登记日期', '登记时间', '入库时间', '出库时间', '姓名', '人数', '事由', '库管员1', '库管员2', '批准人',
          '机构号', '机构名称', '柜员号', '柜员名称', '交易流水号', '维护状态']// 表头
        // 表头对应字段名，要和下面数据key对应
        const filterVal = ['REGISTERDATE', 'RETISTERTIME', 'STOCKINTIME', 'STOCKOUTTIME', 'NAMES', 'NUMBERS',
          'PARTICULAR', 'STOREKEEPER1', 'STOREKEEPER2', 'APPROVER', 'BRNO', 'BRNAME', 'TELLERNO', 'TELLERNAME',
          'MAINSERIALNO', 'WFLAG'
        ]
        const list = this.tableData // 数据来源
        const data = this.formatJson(filterVal, list) // 数据格式化
        var index1 = '出入库房人员登记簿'// 导出时文件名
